...
OWASP_MAP: dict[str, str] = {
    ...
    "deep_login_audit": "A07:2021 Identification & Authentication Failures",
    "root_access_check": "A05:2021 Security Misconfiguration",
    "wp_user_enum": "A01:2021 Broken Access Control",
    "wp_xmlrpc": "A05:2021 Security Misconfiguration",
    "wp_admin_default": "A07:2021 Identification & Authentication Failures",
    "basic_auth_default": "A07:2021 Identification & Authentication Failures",
    "swagger_walker": "A01:2021 Broken Access Control",
    "prometheus_metrics_leak": "A05:2021 Security Misconfiguration",
    "git_repo_dump": "A05:2021 Security Misconfiguration",
    "tomcat_manager_default": "A07:2021 Identification & Authentication Failures",
    "phpmyadmin_default": "A07:2021 Identification & Authentication Failures",
    "adminer_exposed": "A05:2021 Security Misconfiguration",
    "kibana_unauth": "A05:2021 Security Misconfiguration",
    "grafana_default": "A07:2021 Identification & Authentication Failures",
    "ftp_anonymous": "A05:2021 Security Misconfiguration",
    "idor_active_chain": "A01:2021 Broken Access Control",
    "websocket_auth_check": "A05:2021 Security Misconfiguration",
...
MITRE_MAP: dict[str, str] = {
    ...
    "deep_login_audit": "T1110.001 Brute Force: Password Guessing",
    "root_access_check": "T1190 Exploit Public-Facing Application",
    "wp_user_enum": "T1589.002 Email Addresses (user enum)",
    "wp_xmlrpc": "T1498 Network Denial of Service",
    "wp_admin_default": "T1110.001 Brute Force: Password Guessing",
    "basic_auth_default": "T1110.001 Brute Force: Password Guessing",
    "swagger_walker": "T1190 Exploit Public-Facing Application",
    "prometheus_metrics_leak": "T1552.001 Credentials In Files",
    "git_repo_dump": "T1213 Data from Information Repositories",
    "tomcat_manager_default": "T1505.003 Web Shell",
    "phpmyadmin_default": "T1078 Valid Accounts (default)",
    "adminer_exposed": "T1190 Exploit Public-Facing Application",
    "kibana_unauth": "T1213 Data from Information Repositories",
    "grafana_default": "T1078 Valid Accounts (default)",
    "ftp_anonymous": "T1078.001 Default Accounts",
    "idor_active_chain": "T1078 Valid Accounts",
    "websocket_auth_check": "T1185 Browser Session Hijacking",
...
REPRO_MAP: dict[str, str] = {
    ...
    "deep_login_audit": "# curl -sk -X POST '{url}' -d 'username=admin&password=admin' -i  # cek Set-Cookie sesi & redirect",
    "root_access_check": "# curl -sk http://{host}:2375/version  # Docker\n# curl -sk http://{host}:9200/  # Elasticsearch\n# redis-cli -h {host} ping",
    "wp_user_enum": "curl -s {url}/wp-json/wp/v2/users\ncurl -sI '{url}/?author=1'",
    "wp_xmlrpc": "curl -s -X POST {url}/xmlrpc.php -H 'Content-Type: text/xml' \\\n  -d '<methodCall><methodName>system.listMethods</methodName><params/></methodCall>'",
    "wp_admin_default": "curl -i -c /tmp/c -b /tmp/c -d 'log=admin&pwd=admin&wp-submit=Log+In' {url}/wp-login.php",
    "basic_auth_default": "for c in admin:admin tomcat:tomcat root:root; do\n  curl -i -u $c {url}/admin/ | grep -E '^HTTP|Location'\ndone",
    "swagger_walker": "curl -s {url}/v3/api-docs | jq '.paths | keys'\n# lalu uji tiap path tanpa Authorization header",
    "prometheus_metrics_leak": "curl -s {url}/actuator/env | grep -i 'password\\|secret'\ncurl -s {url}/actuator/heapdump -o heap.bin",
    "git_repo_dump": "curl -i {url}/.git/HEAD\ncurl -i {url}/.git/config\nwget --mirror --no-parent {url}/.git/",
    "tomcat_manager_default": "curl -i -u tomcat:tomcat {url}/manager/html\ncurl -s -u tomcat:tomcat {url}/manager/text/list",
    "phpmyadmin_default": "curl -i -d 'pma_username=root&pma_password=&server=1' {url}/index.php",
    "adminer_exposed": "curl -s {url}/adminer.php | grep -i adminer\ncurl -i -d 'auth[driver]=server&auth[server]=127.0.0.1&auth[username]=root&auth[password]=' {url}/adminer.php",
    "kibana_unauth": "curl -s {url}/api/status | jq .version\ncurl -s {url}/_cat/indices?format=json",
    "grafana_default": "curl -s {url}/api/health | jq .version\ncurl -i -X POST {url}/login -H 'Content-Type: application/json' -d '{{\"user\":\"admin\",\"password\":\"admin\"}}'",
    "ftp_anonymous": "ftp -inv {host} <<EOF\nuser anonymous test@test\nls\nbye\nEOF",
    "idor_active_chain": "# Bandingkan response untuk dua ID berbeda:\ncurl -s '{url}' -o /tmp/a\ncurl -s '{url}' | sed 's/id=1/id=2/' -o /tmp/b\ndiff /tmp/a /tmp/b",
    "websocket_auth_check": "wscat -c '{url}' --header 'Origin: https://attacker.invalid'\n# Kalau berhasil 101 + bisa kirim message = CSWSH",
