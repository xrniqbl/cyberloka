...
Output:
  ...
  - Daftar isi.
  - Bab 1 Ringkasan Eksekutif + grafik batang severity.
...
def _slug_host(target: Target) -> str:
    """Slugify target hostname for safe filenames."""
...
def auto_pdf_path(target: Target, out_dir: str | None = None) -> str:
    """Return automatic PDF file path: cyberloka-report-<host>-<YYYYMMDD-HHMMSS>.pdf."""
...
def _styles() -> dict:
    ...
    out = {
        ...
        "ethics": ParagraphStyle(
            ...
            backColor=colors.HexColor("#FCEFEF"),
            borderPadding=8,
        ),
...
def _on_page(canvas, doc, target_label: str, version: str):
    ...
    canvas.drawString(15 * mm, height - 8 * mm, "CYBERLOKA  -  Laporan Pengujian Keamanan Web")
    ...
    canvas.drawString(
        15 * mm, 10 * mm, f"Cyberloka v{version}  |  RAHASIA - hanya untuk pemilik target"
    )
...
def _severity_chart(counts: dict[Severity, int]) -> Drawing:
    """Custom horizontal bar chart, no chart-library dependency."""
    width, height = 16 * cm, 6 * cm
    d = Drawing(width, height)
    rows = [
        (Severity.CRITICAL, SEV_LABEL_ID[Severity.CRITICAL]),
        (Severity.HIGH, SEV_LABEL_ID[Severity.HIGH]),
        (Severity.MEDIUM, SEV_LABEL_ID[Severity.MEDIUM]),
        (Severity.LOW, SEV_LABEL_ID[Severity.LOW]),
        (Severity.INFO, SEV_LABEL_ID[Severity.INFO]),
    ]
    ...
    for sev, label in rows:
        y -= bar_h + 0.2 * cm
        d.add(
            String(0, y + bar_h / 2 - 4, label, fontName="Helvetica-Bold", fontSize=9)
        )
        d.add(
            Rect(
                3 * cm, y, bar_max, bar_h,
        ...
        d.add(
            String(
                3 * cm + bar_max + 0.2 * cm, y + bar_h / 2 - 4,
                str(n), fontName="Helvetica-Bold", fontSize=10,
                fillColor=PRIMARY,
            )
        )
    return d
...
def _attack_flowchart(target_label: str, modules: list[str]) -> Drawing:
    """Simple flowchart: Recon -> Probe -> Exploit -> Post-Exploit -> Impact."""
    ...
    for title, lines, fill in boxes:
        d.add(
            Rect(x, y, box_w, box_h, fillColor=fill, strokeColor=PRIMARY,
                 strokeWidth=0.7, rx=6, ry=6)
        )
        d.add(
            String(
                x + box_w / 2, y + box_h - 16,
                title, fontName="Helvetica-Bold", fontSize=10,
        ...
        for line in lines:
            ...
            cy -= 11
        centers.append((x + box_w, y + box_h / 2, x + box_w + 0.4 * cm, y + box_h / 2))
        x += box_w + 0.4 * cm

    # arrows
    for i, (x1, y1, x2, y2) in enumerate(centers[:-1]):
        # Arrow line
        d.add(Polygon(
            points=[
                x1, y1 - 3,
...
def _kv_table(rows: list[tuple[str, str]], styles) -> Table:
    ...
    for k, v in rows:
        data.append([
            _para(k, styles["kv_key"]),
            _para(v or "-", styles["kv_val"]),
        ])
...
def _severity_pill(sev: Severity, styles) -> Table:
    label = SEV_LABEL_ID[sev]
    p = Paragraph(
        f"<font color='white'><b>{label}</b></font>", styles["body"]
    )
...
def _findings_overview_table(findings: list[Finding], styles) -> Table:
    ...
    t = Table(
        data,
        colWidths=[1.0 * cm, 2.6 * cm, 3.0 * cm, 10.0 * cm],
        repeatRows=1,
        hAlign="LEFT",
    )
    style_cmds = [
...
def _build(doc_path: str, target: Target, config: ScanConfig, findings: list[Finding]):
    ...
    # Sort findings by severity then module for predictable order
    ...
    story.append(_para(
        "<font color='white'><b>LAPORAN PENGUJIAN<br/>KEAMANAN WEB</b></font>",
        styles["title"],
    ))
    story.append(Spacer(1, 0.3 * cm))
    story.append(_para(
        "<font color='white'>Penilaian Kerentanan & Rekomendasi Perbaikan</font>",
        styles["subtitle"],
    ))
    ...
    cover_rows = [
        ...
        (
            "Severity Tertinggi",
            (
                SEV_LABEL_ID[findings_sorted[0].severity]
                if findings_sorted else "-"
            ),
        ),
    ]
    ...
    if findings_sorted:
        ...
        story.append(_findings_overview_table(top5, styles))

    # =================== 2. FLOWCHART ALUR SERANGAN =======================
    story.append(PageBreak())
    ...
    if not accesses:
        ...
        for label, items in grouped.items():
            ...
            for f in items:
                evidence_short = (f.evidence or "-").splitlines()[0][:140] if f.evidence else "-"
                data.append([
                    _severity_pill(f.severity, styles),
                    _para(f.module, styles["body"]),
                    _para(f"{f.target}<br/><font size='8' color='#7F8C8D'>{evidence_short}</font>",
                          styles["body"]),
                ])
    ...
    story.append(_para(
        "Setiap temuan dijelaskan dengan empat bagian utama: <b>Apa Celahnya</b> "
        "(akar masalah teknis), <b>Bagaimana Hacker Membobolnya</b> (skenario eksploitasi), "
        "<b>Bukti</b> (dari hasil scan), dan <b>Cara Menanggulangi</b> (langkah perbaikan).",
        styles["body"],
    ))
    story.append(Spacer(1, 0.3 * cm))
    ...
    for i, f in enumerate(findings_sorted, 1):
        ...
        story.append(_para(sc.get("what", ""), styles["body"]))
        if f.description:
        ...
        mitigation = sc.get("mitigate", "") or f.remediation
        story.append(_para(mitigation, styles["body"]))
        if f.remediation and sc.get("mitigate") and f.remediation.strip() != sc.get("mitigate", "").strip():
            story.append(_para("<b>Remediasi spesifik dari modul:</b>", styles["body"]))
            story.append(_para(f.remediation, styles["body"]))
    ...
    rec_rows = [[
        _para("<b>Prioritas</b>", styles["body"]),
