...
def iter_param_urls(url: str, payload: str):
    """Yield (param, mutated_url) for each query parameter, replacing value with payload."""
    parsed = urlparse(url)
...
def replace_param(url: str, key: str, value: str) -> str:
    """Replace ``key`` value in ``url`` (or append if missing)."""
    parsed = urlparse(url)
    params = parse_qsl(parsed.query, keep_blank_values=True)
...
def candidate_params(
    url: str,
    hints: Iterable[str] = ("id", "q", "search", "page", "file", "name"),
) -> list[str]:
...
# ---------------------------------------------------------------------------
# Baseline / diff helpers - used by injection modules to filter false positives
# ---------------------------------------------------------------------------
def baseline(client: HttpClient, url: str, samples: int = 2) -> dict | None:
    """Capture baseline response for ``url`` (status, length, latency, body)."""
    statuses: list[int] = []
    lengths: list[int] = []
...
def diff_significant(base: dict | None, resp_status: int, resp_text: str) -> bool:
    """True when the new response materially differs from the baseline."""
    if base is None:
        return False
