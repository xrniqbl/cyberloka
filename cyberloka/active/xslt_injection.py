...
Masalah versi lama (false positive CRITICAL):
  ...
  temuan dilaporkan CRITICAL padahal hanya refleksi input (prediksi, bukan bukti).

Pendekatan baru — kebal refleksi (prinsip sama dengan modul `cmdi`):
  Stylesheet menjumlahkan dua angka acak lewat `<xsl:value-of select="A+B"/>` yang
  dibungkus marker unik `CLK<hasil>END`. Hanya dilaporkan bila respons memuat HASIL
  penjumlahan (string yang TIDAK pernah ada di teks payload) DAN ekspresi/markup XSL
  mentah TIDAK terpantul. Server yang sekadar memantulkan input mengembalikan ekspresi
  mentah `<xsl:value-of select="A+B"/>`, bukan hasilnya, sehingga tidak mungkin memicu
  temuan. Match HANYA terjadi bila processor XSLT benar-benar mengevaluasi stylesheet
  → bukti pasti, bukan tebakan.

  Catatan: dipakai PENJUMLAHAN dua angka 6-digit (bukan perkalian). XSLT 1.0
  memformat angka besar (>~10 digit) dalam notasi ilmiah (mis. `4.16e+09`), sehingga
  hasil perkalian bisa gagal dicocokkan persis. Jumlah dua angka 6-digit selalu
  <2.000.000 → di-render sebagai integer biasa oleh processor XSLT.
"""
...
def _is_executed(body: str, expected: str, literal_expr: str) -> bool:
    ...
    - `expected` (CLK<A+B>END) hadir: hasil penjumlahan, mustahil dari refleksi karena
